...
class ContactModel extends DB
{
  ...
  {
    $query = "SELECT COUNT(ID) FROM contact WHERE 1";
  ...
  public function getAllContactsWLimit($offset, $limit)
  {
    $query = "SELECT ID, Name, Telephone, Email, Message_content, Created, Status FROM contact ORDER BY Status ASC,Created DESC LIMIT ? OFFSET ?";
    $stmt = mysqli_stmt_init($this->con);
    mysqli_stmt_prepare($stmt, $query);
